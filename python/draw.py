...
class Draw_MPC_point_stabilization_v1(object):
    def __init__(
        ...
        self.fig = plt.figure()
        self.ax = plt.axes(xlim=(-0.8, 3), ylim=(-0.8, 3.0))
    ...
    def animation_init(self):
        ...
        self.target_circle = plt.Circle(
            self.target_state[:2], self.rob_radius, color="b", fill=False
        )
        ...
        self.robot_body = plt.Circle(
            self.init_state[:2], self.rob_radius, color="r", fill=False
        )
...
class Draw_MPC_Obstacle(object):
    ...
    def animation_init(self):
        ...
        self.obstacle_circle = plt.Circle(
            self.obstacle[:2], self.obstacle[2], color="g", fill=True
        )
...
class Draw_MPC_tracking(object):
    def __init__(
        self, robot_states: list, init_state: np.array, rob_diam=0.3, export_fig=False
    ):
        self.init_state = init_state
        self.robot_states = robot_states
    ...
    def animation_init(
        ...
        self.init_robot_position = plt.Circle(
            self.init_state[:2], self.rob_radius, color="r", fill=False
        )
